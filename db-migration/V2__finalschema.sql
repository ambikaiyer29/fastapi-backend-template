...
CREATE TABLE IF NOT EXISTS "fastapiSK".users (
    ...
    tenant_id UUID NOT NULL REFERENCES "fastapiSK".tenants(id) ON DELETE CASCADE,
    user_data JSONB, -- Formerly 'metadata'
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    created_by UUID,
    updated_by UUID
...
-- -----------------------------------------------------------------
-- Section 4: Application Role and Permissions
-- This section is CRITICAL for security. It creates the non-privileged
-- role that our FastAPI application will use to connect to the DB.
-- -----------------------------------------------------------------

-- Create the dedicated, non-privileged role for the API if it doesn't exist.
DO
$do$
BEGIN
   IF NOT EXISTS (
      ...
      WITH LOGIN
      PASSWORD 'YourStrongAndUniquePasswordHere' -- IMPORTANT: CHANGE THIS PASSWORD
      NOSUPERUSER
      NOBYPASSRLS;
   END IF;
END
...
-- Drop all existing policies to ensure a clean slate before recreating them.
...
DROP POLICY IF EXISTS "user_can_select_self_policy" ON "fastapiSK".users;
...
DROP POLICY IF EXISTS "superadmin_bypass_policy" ON "fastapiSK".audit_logs;
DROP POLICY IF EXISTS "tenant_isolation_policy" ON "fastapiSK".audit_logs;
...
DROP POLICY IF EXISTS "superadmin_bypass_policy" ON "fastapiSK".api_keys;
DROP POLICY IF EXISTS "tenant_isolation_policy" ON "fastapiSK".api_keys;
...
DROP POLICY IF EXISTS "tenant_isolation_policy" ON "fastapiSK".custom_objects;
DROP POLICY IF EXISTS "superadmin_bypass_policy" ON "fastapiSK".custom_objects;
...
DROP POLICY IF EXISTS "tenant_isolation_policy" ON "fastapiSK".custom_fields;
DROP POLICY IF EXISTS "superadmin_bypass_policy" ON "fastapiSK".custom_fields;
...
DROP POLICY IF EXISTS "tenant_isolation_policy" ON "fastapiSK".records;
DROP POLICY IF EXISTS "superadmin_bypass_policy" ON "fastapiSK".records;
...
DROP POLICY IF EXISTS "allow_read_for_all" ON "fastapiSK".plans;
CREATE POLICY "allow_read_for_all" ON "fastapiSK".plans FOR SELECT USING (true);
...
DROP POLICY IF EXISTS "allow_read_for_all" ON "fastapiSK".plan_entitlements;
CREATE POLICY "allow_read_for_all" ON "fastapiSK".plan_entitlements FOR SELECT USING (true);
...
DROP POLICY IF EXISTS "tenant_isolation_policy" ON "fastapiSK".usage_records;
DROP POLICY IF EXISTS "superadmin_bypass_policy" ON "fastapiSK".usage_records;
...
-- =================================================================
--    DATABASE MIGRATION: Fix RLS Policies for Plans & Entitlements
-- =================================================================

-- Drop the old, incomplete policies
DROP POLICY IF EXISTS "allow_read_for_all" ON "fastapiSK".plans;
DROP POLICY IF EXISTS "superadmin_full_access" ON "fastapiSK".plans;

DROP POLICY IF EXISTS "allow_read_for_all" ON "fastapiSK".plan_entitlements;
DROP POLICY IF EXISTS "superadmin_full_access" ON "fastapiSK".plan_entitlements;
...
DROP POLICY IF EXISTS "tenant_isolation_policy" ON "fastapiSK".checkout_sessions;
DROP POLICY IF EXISTS "superadmin_bypass_policy" ON "fastapiSK".checkout_sessions;
